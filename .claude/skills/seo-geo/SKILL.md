...
Miroir Claude de `.agents/skills/seo-geo/SKILL.md`. Même geste. En cas d’écart, le fichier `.agents` gagne — aligne les deux.
...
2. **Dernières captures** — `docs/visibility/captures/` si le dossier existe. Le check [`ai-search-visibility`](../../../.agents/skills/ai-search-visibility/SKILL.md) peut aussi avoir ajouté au bus.
...
7. **Finir par `/end`.** Lire [`.agents/skills/end/SKILL.md`](../../../.agents/skills/end/SKILL.md) et enchaîner. Ne pas redemander. C’est le land sur `main`.
