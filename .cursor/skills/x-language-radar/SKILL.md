...
Copy X (si le brief en demande) : voix **fondateur** — Ed., I / we built 3xrep. Copy site (home, `/install`, README) : produit, sauf si le fondateur demande un visage.
...
- Brouillons jusqu’à compte X utilisable + org payante `/install` + go explicite. Pas de message privé auto. Voix fondateur sur X (Ed. / I / we built 3xrep). Lien doux `https://3xrep.com/install`. Prix **$129 / org / mois**.
...
5. **Écrire la reco.** Pas le patch. Pas `seo-geo`. Pas `/end` sur du copy live. Si une phrase X est jointe : première personne fondateur.
...
X (si joint) : Ed. / I / we built 3xrep
